...
/**
 * This Signature class is used to provide applications the functionality of a
...
public class Signature {
   ...
   * 
   * @param algorithm
   *          the standard string name of the algorithm. See Appendix A in the
   *          <a href=
...
//      new ServiceId("Cipher", "RSA"), });
   ...
   * with the most preferred Provider. A new Signature object encapsulating the
   ...
   * 
   * @param algorithm
   *          the standard name of the algorithm requested. See Appendix A in
   *          the <a href=
   *          "../../../technotes/guides/security/crypto/CryptoSpec.html#AppA">
   *          Java Cryptography Architecture API Specification &amp; Reference
   *          </a> for information about standard algorithm names.
...
//  }
   ...
   * 
   * @param algorithm
   *          the name of the algorithm requested. See Appendix A in the <a
   *          href=
   *          "../../../technotes/guides/security/crypto/CryptoSpec.html#AppA">
   *          Java Cryptography Architecture API Specification &amp; Reference
   *          </a> for information about standard algorithm names.
   ...
   * 
   * @exception NoSuchAlgorithmException
   *              if a SignatureSpi implementation for the specified algorithm
...
//      throw new NoSuchAlgorithmException("no such algorithm: " + RSA_SIGNATURE
...
//  }
   ...
   * field implies that the public key in the certificate and its corresponding
   * private key are not supposed to be used for digital signatures, an
   ...
   * 
   * @exception InvalidKeyException
   *              if the public key in the certificate is not encoded properly
   *              or does not include required parameter information or cannot
   *              be used for digital signature purposes.
   * @since 1.3
   */
...
//  }
   ...
   * via new calls to <code>update</code> and <code>sign</code>.
   * 
...
//  }
   ...
   * in the provided buffer <code>outbuf</code>, starting at <code>offset</code>
...
//  }
  ...
  /**
   * Verifies the passed-in signature in the specified array of bytes, starting
...
//  }
   ...
   * Processes the <code>data.remaining()</code> bytes starting at at
   * <code>data.position()</code>. Upon return, the buffer's position will be
...
//  }
   ...
   *              if <code>param</code> is an invalid parameter for this
   *              signature algorithm engine, the parameter is already set and
   *              cannot be set again, a security exception occurs, and so on.
   * 
   * @see #getParameter
   * 
   * @deprecated Use
...
//  }
   ...
   * 
   * @return the object that represents the parameter value, or null if there is
   *         none.
   * 
   * @exception InvalidParameterException
   *              if <code>param</code> is an invalid parameter for this engine,
...
//  }
   ...
   * 
   * Note: All SPI methods from the original Signature class have been moved up
   * the hierarchy into a new class (SignatureSpi), which has been interposed in
   * the hierarchy between the API (Signature) and its original parent (Object).
   */
...
//     * provider selection is not possible because initSign()/ initVerify() is
//     * not the first method called.
//     */
//    void chooseFirstProvider() {
//      if (sigSpi != null) {
//        return;
...
//            "No installed provider supports this key: " + k, lastException);
//      }
//    }
//
