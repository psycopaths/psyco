...
public final class InterpolationUtil {
  ...
  public InterpolationUtil(InterpolationSolver iSolver,
  ...
  /**
   * expands future after a prefix for some depth and computes an interpolant
   * characterizing program states for which the model is conformant after the
   * state reached by the prefix for this depth.
   *
   * @param prefix
   * @param path
   * @param depth
  ...
  public Expression<Boolean> expand(Word<SymbolicMethodSymbol> prefix,
    ...
    Expression<Boolean> itp = cache.lookup(depth, state,
            InterpolationUtil.asExpression(
  ...
  private Expression<Boolean> expand(Word<SymbolicMethodSymbol> prefix,
          Word<Path> path, SymbolicMethodSymbol a, int depth)
          throws CounterexampleFound {
    ...
    for (Path p : summary) {
      ...
      if (cSolver.isSatisfiable(p.getPathCondition())
      ...
      if (sat && !conforms) {
        ...
        Expression<Boolean> itp
                = interpolate(prefix, path, a, p.getPathCondition());
        interpolants.add(itp);
      } // conforms
      else if (sat && conforms) {
        if (depth > 0 && p.getState().equals(PathState.OK)) {
          Expression<Boolean> itp = expand(nextPrefix, nextPath, depth);
          Expression<Boolean> error
                  = InterpolationUtil.makeErrorCondition(p, itp);
  ...
  /**
   * calls the interpolation solver to create an interpolant between (prefix,
   * path) and the unsatisfiable and unconformat paths of a.
   *
   * @param prefix
   * @param path
   * @param a
   ...
   * @return
   */
  public Expression<Boolean> interpolate(Word<SymbolicMethodSymbol> prefix,
          Word<Path> path, SymbolicMethodSymbol a,
    ...
    PsycoProfiler.start("InterpolationSolver");
    List<Expression<Boolean>> itp = iSolver.getInterpolants(terms);
    PsycoProfiler.stop("InterpolationSolver");
    return itp.get(0);
  }

  /**
   * creates an error condition from a path with precondition p and
   * postcondition q and an interpolant over-approximating OK paths after (p,q).
   *
   ...
   * @return
   */
  public static Expression<Boolean> asExpression(
          Word<SymbolicMethodSymbol> prefix,
